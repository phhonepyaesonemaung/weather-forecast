"""
Train the feature-attention-first dual-attention LSTM for Mandalay temperature
forecasting -- PyTorch version.

Architecture: Input -> Feature Attention -> LSTM -> Temporal Attention -> Prediction

Run from the weather-forecast/ project root:
    python train_feature_first_attention_torch.py
    python train_feature_first_attention_torch.py --seed 7 --epochs 100 --patience 15

Expects data/20years_dataset_mandalay.csv relative to where you run this from.
Saves the trained model to models/feature_first_attention_seed<seed>.pt
"""
import argparse
import copy
import json
...
# Applied BEFORE the LSTM in this version.
# -----------------------------------------------------------------
...
# time axis, applied to the LSTM's output sequence.
# -----------------------------------------------------------------
...
class FeatureFirstDualAttentionLSTM(nn.Module):
    ...
    def forward(self, x):
        weighted_x, feat_weights = self.feature_attention(x)
        lstm_out, _ = self.lstm(weighted_x)
        lstm_out = self.dropout1(lstm_out)
        context, temp_weights = self.temporal_attention(lstm_out)
...
def main():
    ...
    model = FeatureFirstDualAttentionLSTM(n_features).to(device)
    print(model)
    ...
    model_path = os.path.join(args.output_dir, f"feature_first_attention_seed{args.seed}.pt")
    torch.save(model.state_dict(), model_path)
    print(f"Saved model to {model_path}")

    result = {"model": "feature_first_torch", "seed": args.seed, "mae": mae, "rmse": rmse, "naive_mae": naive_mae}
