"""
Train the feature-attention-first dual-attention LSTM for Mandalay temperature forecasting.

Architecture: Input -> Feature Attention -> LSTM -> Temporal Attention -> Prediction

Run from the weather-forecast/ project root:
    python train_feature_first_attention.py
    python train_feature_first_attention.py --seed 7 --epochs 100 --patience 15

Expects data/20years_dataset_mandalay.csv relative to where you run this from.
Saves the trained model to models/feature_first_attention_seed<seed>.keras
"""
import argparse
import json
import os
...
def main():
    ...
    # -----------------------------------------------------------------
    # Feature Attention layer: per-timestep softmax over the input's
    # feature axis. Applied BEFORE the LSTM in this version.
    # -----------------------------------------------------------------
    ...
    # the time axis, applied to the LSTM's output sequence.
    # -----------------------------------------------------------------
    ...
    lstm_out = layers.LSTM(64, return_sequences=True, name="lstm")(weighted_inputs)
    lstm_out = layers.Dropout(0.2)(lstm_out)
    context, temp_weights = TemporalAttention(units=32, name="temporal_attention")(lstm_out)
    x = layers.Dense(16, activation="relu")(context)
    x = layers.Dropout(0.2)(x)
    output = layers.Dense(1, name="prediction")(x)

    model = keras.Model(inputs, output, name="feature_first_dual_attention")
    model.compile(
    ...
    print(f"\nFeature-first Dual-Attention LSTM -> MAE: {mae:.3f} C   RMSE: {rmse:.3f} C")
    print(f"Naive persistence baseline        -> MAE: {naive_mae:.3f} C")

    os.makedirs(args.output_dir, exist_ok=True)
    model_path = os.path.join(args.output_dir, f"feature_first_attention_seed{args.seed}.keras")
    model.save(model_path)
    print(f"Saved model to {model_path}")

    result = {"model": "feature_first", "seed": args.seed, "mae": mae, "rmse": rmse, "naive_mae": naive_mae}
